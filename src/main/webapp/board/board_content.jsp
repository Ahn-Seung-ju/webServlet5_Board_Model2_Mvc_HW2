				...
				<table width="80%" border="1">
					...
					<tr>
						<td colspan="4" align="center">
							<a href="Boardlist.board?cp=<%=cpage%>&ps=<%=pagesize%>">목록가기</a> 
							<a href="Boardedit.board?idx=<%=idx%>&cp=<%=cpage%>&ps=<%=pagesize%>">편집</a>
							<a href="Boarddelete.board?idx=<%=idx%>&cp=<%=cpage%>&ps=<%=pagesize%>">삭제</a>
							<a href="Boardrewrite.board?idx=<%=idx%>&cp=<%=cpage%>&ps=<%=pagesize%>&subject=<%=board.getSubject()%>">답글</a>
						</td>
					</tr>
				</table>
				<!--  꼬리글 달기 테이블 -->
				...
				<!-- 꼬리글 목록 테이블 -->
					...
					%>
						<tr align="left">
							...
							<td width="20%">
							<form action="Boardreplydeletok.board" method="POST" name="replyDel">
